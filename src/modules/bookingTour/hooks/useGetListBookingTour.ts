...
const parseRequest = (filters: FormFilterTourBooking) => {
  return TourBookingModel.parseBodyToSearch(filters);
};

const useGetListBookingTour = (
  filters: FormFilterTourBooking,
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          const response = await BookingTourServices.getListBookingTour(nextFilters);
          resolve(response);
        } catch (error) {
          setError(error);
          reject(error);
...
export default useGetListBookingTour;

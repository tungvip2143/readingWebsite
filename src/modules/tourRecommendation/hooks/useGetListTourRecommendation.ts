...
import tourRecommendationServices from '../tourRecommendation.services';

const parseRequest = (filters: ResquestGetListTourRecommendation) => {
  return cloneDeep({
...
const useGetListTourRecommendation = (
  filters: ResquestGetListTourRecommendation,
  ...
  const save = useSave();
  const [data, setData] = useState<TourRecommendationList>();
  ...
  //! Function
  const fetch: () => Promise<ResponseGetListTourRecommendation> | undefined = useCallback(() => {
    if (!isTrigger) {
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          const response = await tourRecommendationServices.getListTourRecommendation(nextFilters);
          resolve(response);
        } catch (error) {
          setError(error);
          reject(error);
  ...
  const checkConditionPass = useCallback(
    (response: ResponseGetListTourRecommendation) => {
...
export default useGetListTourRecommendation;

...
const ProfileSetting = (props: ProfileSettingProps) => {
  ...
  const renderInfoStatus = () => {
    if (dataProfile?.status === TourGuideInfoStatus.DISAPPROVAL) {
      return t('Profile.unapprovedAccount');
    }
    if (dataProfile?.status === TourGuideInfoStatus.VERIFIED) {
      return t('Profile.approvedAccount');
    }
    if (dataProfile?.status === TourGuideInfoStatus.PENDING_APPROVAL) {
      return t('Profile.pendingApproveAccount');
    }
    if (dataProfile?.status === TourGuideInfoStatus.NEW) {
      return t('Profile.newAccount');
    }
    return null;
  };
  //! Render

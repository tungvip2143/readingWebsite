...
const parseRequest = (filters: RequestGetListVendorSubscribe) => {
  return cloneDeep({
    page: filters.page,
...
const useGetListVendorSubscribe = (
  tourId: number,
  filters: RequestGetListVendorSubscribe,
  ...
  const save = useSave();
  const [data, setData] = useState<ResponseGetListVendorSubscribe>();
  ...
  //! Function
  const fetch: () => Promise<ResponseGetListVendorSubscribe> | undefined = useCallback(() => {
    if (!isTrigger) {
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          const response = await VendorSubscribeServices.getListVendorSubscribe(
            tourId,
            nextFilters
          );
          resolve(response);
  ...
  const checkConditionPass = useCallback(
    (response: ResponseGetListVendorSubscribe) => {
  ...
  const refetch = useCallback(async () => {
    try {
      setRefetching(true);
      const nextFilters = parseRequest(filters);
      const response = await VendorSubscribeServices.getListVendorSubscribe(tourId, nextFilters);
      checkConditionPass(response);
      setRefetching(false);
    } catch (error: any) {
      if (!error.isCanceled) {
...
export default useGetListVendorSubscribe;

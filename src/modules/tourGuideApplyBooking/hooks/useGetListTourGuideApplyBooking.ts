...
const parseRequest = (filters: RequestGetListTourGuideApplyBooking) => {
  return cloneDeep({
    page: filters.page,
...
const useGetListTourGuideApplyBooking = (
  filters: RequestGetListTourGuideApplyBooking,
  ...
  //! Function
  const fetch: () => Promise<ResponseGetListTourGuideApplyBooking> | undefined = useCallback(() => {
    if (!isTrigger) {
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          const response = await TourGuideApplyBookingServices.getListTourGuideApplyBooking(
            nextFilters
          );
          resolve(response);
        } catch (error) {
  ...
  const checkConditionPass = useCallback(
    (response: ResponseGetListTourGuideApplyBooking) => {
...
export default useGetListTourGuideApplyBooking;

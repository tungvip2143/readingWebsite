...
import { Review, Vendor } from 'modules/vendor/vendor.interface';
import moment from 'moment';
import cachedKeys from 'constants/cachedKeys';

interface ReviewsPlacesDetailContainerProps {}
...
const ReviewsPlacesDetailContainer = (props: ReviewsPlacesDetailContainerProps) => {
  //! State
  const theme = useTheme();
  ...
  const ReviewList = () => {
    ...
    return (
      <CommonStylesClient.Box sx={{ display: 'flex', gap: '1.5rem', flexDirection: 'column' }}>
        {renderReviewList.map((item: Review, index: number) => {
          return (
            <Review
...
export default React.memo(ReviewsPlacesDetailContainer);

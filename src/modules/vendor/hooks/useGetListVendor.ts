...
import { RequestGetListVendor, ResponseGetListVendor } from '../vendor.interface';
...
const useGetListVendor = (
  ...
  const save = useSave();
  const [data, setData] = useState<ResponseGetListVendor>();
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          const response = await VendorServices.getListVendor(nextFilters);
          resolve(response);
        } catch (error) {
          setError(error);
          reject(error);
...
export default useGetListVendor;

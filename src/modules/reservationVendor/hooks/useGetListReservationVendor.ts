...
import ReservationVendorModel from 'models/reservationVendor.model';
...
const parseRequest = (filters: GetListReservationVendor) => {
  return ReservationVendorModel.parseBodyToRequest(filters);
};

const useGetListReservationVendor = (
  filters: GetListReservationVendor,
  ...
  const save = useSave();
  const [data, setData] = useState<ResponseGetListReservationVendor>();
  ...
  //! Function
  const fetch: () => Promise<ResponseGetListReservationVendor> | undefined = useCallback(() => {
    if (!isTrigger) {
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          const response = await ReservationVendorServices.getListReservationVendor(nextFilters);
          resolve(response);
        } catch (error) {
          setError(error);
          reject(error);
  ...
  const checkConditionPass = useCallback(
    (response: ResponseGetListReservationVendor) => {
...
export default useGetListReservationVendor;

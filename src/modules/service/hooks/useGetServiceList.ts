...
const parseRequest = (filters: RequestGetListService) => {
  return cloneDeep({
    page: filters.page,
...
const useGetServiceList = (
  ...
  //! Function
  const fetch: () => Promise<ResponseGetListService> | undefined = useCallback(() => {
    if (!isTrigger) {
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          const response = await ServiceServices.getListService(nextFilters);
          resolve(response);
        } catch (error) {
          setError(error);
          reject(error);
  ...
  const checkConditionPass = useCallback(
    (response: ResponseGetListService) => {

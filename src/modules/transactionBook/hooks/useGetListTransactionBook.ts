...
import { RequestGetListTransactionBook, ResponseGetListTransactionBook } from '../transactionBook.interface';
...
const parseRequest = (filters: RequestGetListTransactionBook) => {
  return cloneDeep({
    page: filters.page,
...
const useGetListTransactionBook = (
  filters: RequestGetListTransactionBook,
  ...
  const save = useSave();
  const [data, setData] = useState<ResponseGetListTransactionBook>();
  ...
  //! Function
  const fetch: () => Promise<ResponseGetListTransactionBook> | undefined = useCallback(() => {
    if (!isTrigger) {
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          const response = await TransactionBookServices.getListTransactionBook(nextFilters);
          resolve(response);
        } catch (error) {
          setError(error);
          reject(error);
  ...
  const checkConditionPass = useCallback(
    (response: ResponseGetListTransactionBook) => {
...
export default useGetListTransactionBook;

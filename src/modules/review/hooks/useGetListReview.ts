...
import { RequestGetListReview, ResponseGetListReview } from '../review.interface';
...
const parseRequest = (filters: RequestGetListReview) => {
  return cloneDeep({
    page: filters.page,
...
const useGetListReview = (
  ...
  const save = useSave();
  const [data, setData] = useState<ResponseGetListReview>();
  ...
  //! Function
  const fetch: () => Promise<ResponseGetListReview> | undefined = useCallback(() => {
    if (!isTrigger) {
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          const response = await ReviewServices.getListReview(nextFilters);
          resolve(response);
        } catch (error) {
          setError(error);
          reject(error);
  ...
  const checkConditionPass = useCallback(
    (response: ResponseGetListReview) => {

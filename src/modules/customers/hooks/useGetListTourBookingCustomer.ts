...
const parseRequest = (filters: RequestGetTourBookingListCustomer) => {
  return cloneDeep({
    page: filters.page,
...
const useGetCustomerList = (id: number | string,
  filters: RequestGetTourBookingListCustomer,
  ...
  const save = useSave();
  const [data, setData] = useState<TourBookingList>();
  ...
  //! Function
  const fetch: () => Promise<ResponseGetTourBookingListCustomer> | undefined = useCallback(() => {
    if (!isTrigger) {
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          const response = await CustomerService.getTourBookingList({id: id, body: nextFilters});
          resolve(response);
        } catch (error) {
          setError(error);
          reject(error);
  ...
  const checkConditionPass = useCallback(
    (response: ResponseGetTourBookingListCustomer) => {

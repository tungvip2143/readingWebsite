...
const parseRequest = (filters: RequestGetBookList) => {
  return cloneDeep({
    page: filters.page,
    limit: filters.limit,
  });
...
const useGetBookList = (
  ...
  //! Function
  const fetch: () => Promise<ResponseGetBookListPaging> | undefined = useCallback(() => {
    if (!isTrigger) {
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          const response = await bookServices.getBookListPaging(nextFilters);
          resolve(response);
        } catch (error) {
          setError(error);
          reject(error);
  ...
  const checkConditionPass = useCallback(
    (response: ResponseGetBookListPaging) => {

...
import TourGuideSubscribeServices from '../tourGuideSubscribe.services';
...
const parseRequest = (filters: RequestGetListTourGuideSubscribe) => {
  return cloneDeep({
    page: filters.page,
...
const useGetListTourGuideSubscribe = (
  tourId: number,
  filters: RequestGetListTourGuideSubscribe,
  ...
  const save = useSave();
  const [data, setData] = useState<ResponseGetListTourGuideSubscribe>();
  ...
  //! Function
  const fetch: () => Promise<ResponseGetListTourGuideSubscribe> | undefined = useCallback(() => {
    if (!isTrigger) {
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          const response = await TourGuideSubscribeServices.getListTourGuideSubscribe(
            tourId,
            nextFilters
          );
          resolve(response);
  ...
  }, [filters, tourId, isTrigger]);

  const checkConditionPass = useCallback(
    (response: ResponseGetListTourGuideSubscribe) => {
...
export default useGetListTourGuideSubscribe;

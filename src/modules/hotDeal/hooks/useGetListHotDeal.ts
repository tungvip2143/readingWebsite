...
import { RequestGetListHotDeal, ResponseGetListHotDeal } from '../hotDeal.interface';
...
const parseRequest = (filters: RequestGetListHotDeal) => {
  return cloneDeep({
    page: filters.page,
...
const useGetListHotDeal = (
  filters: RequestGetListHotDeal,
  ...
  const save = useSave();
  const [data, setData] = useState<ResponseGetListHotDeal>();
  ...
  //! Function
  const fetch: () => Promise<ResponseGetListHotDeal> | undefined = useCallback(() => {
    if (!isTrigger) {
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          const response = await HotDealServices.getListHotDeal(nextFilters);
          resolve(response);
        } catch (error) {
          setError(error);
          reject(error);
  ...
  const checkConditionPass = useCallback(
    (response: ResponseGetListHotDeal) => {
...
export default useGetListHotDeal;

...
const parseRequest = (filters: RequestGetListGetProfileCustomer) => {
  return cloneDeep({
    page: filters.page,
...
const useGetListGetProfileCustomer = (
  filters: RequestGetListGetProfileCustomer,
  ...
  const save = useSave();
  const [data, setData] = useState<ResponseGetListGetProfileCustomer>();
  ...
  //! Function
  const fetch: () => Promise<ResponseGetListGetProfileCustomer> | undefined = useCallback(() => {
    if (!isTrigger) {
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          const response = await GetProfileCustomerServices.getListGetProfileCustomer(nextFilters);
          resolve(response);
        } catch (error) {
          setError(error);
          reject(error);
  ...
  const checkConditionPass = useCallback(
    (response: ResponseGetListGetProfileCustomer) => {
...
export default useGetListGetProfileCustomer;

...
const parseRequest = (filters: RequestGetListWishListVendor) => {
  return cloneDeep({
    page: filters.page,
...
const useGetListWishListVendor = (
  filters: RequestGetListWishListVendor,
  ...
  const save = useSave();
  const [data, setData] = useState<ResponseGetListWishListVendor>();
  ...
  //! Function
  const fetch: () => Promise<ResponseGetListWishListVendor> | undefined = useCallback(() => {
    if (!isTrigger) {
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          const response = await WishListVendorServices.getListWishListVendor(nextFilters);
          resolve(response);
        } catch (error) {
          setError(error);
          reject(error);
  ...
  const checkConditionPass = useCallback(
    (response: ResponseGetListWishListVendor) => {
...
export default useGetListWishListVendor;
